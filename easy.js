...
const altNumbers = (numArray) => {
    ...
    let posArray = []
    let negArray = []
    // Split the given array by positive and negative numbers
    for (let i = 0; i < numArray.length; i++){
        if (numArray[i] < 0) negArray.push(numArray[i])
        else posArray.push(numArray[i])
    }
    let result = []
    // Find the array that is smaller
    let l = Math.min(posArray.length, negArray.length);
    for (let i = 0; i < l; i++) {
        // The larger array will start for the result array
        if (l === negArray.length) result.push(posArray[i], negArray[i]);
        else result.push(negArray[i], posArray[i]);
    }
    return result;
}
